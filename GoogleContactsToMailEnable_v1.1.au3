...
Local $aUtilVer = "v1.1" ; (2021-02-23)
...
Func _ConvertGoogleToMailEnable()
	...
	For $tNo = 0 To ($aNumberOfContacts - 1)
		$tCnt += 1
		If $tCnt = 10 Then
			ControlSetText($aSplash, "", "Static1", "Converting to MailEnable files." & @CRLF & "Processing contact " & $tNo & " of " & $aNumberOfContacts & _
					@CRLF & @CRLF & "Timer:" & Round(_Timer_Diff($aTimerStart) / 1000) & "s")
			$tCnt = 0
		EndIf
		Local $tAdd2Home = ""
...
Func _WriteMailEnable()
	For $tNo = 0 To ($aNumberOfContacts - 1)
		Local $tArray[0]
		ControlSetText($aSplash, "", "Static1", "Writing MailEnable files." & @CRLF & "Processing contact " & $tNo & " of " & $aNumberOfContacts & _
				@CRLF & @CRLF & "Timer:" & Round(_Timer_Diff($aTimerStart) / 1000) & "s")
...
;~ 		If   <> "" Then $tAGENT = '<AGENT>' &  & '</AGENT>'
		...
		$mIndex[$tNo] = $tELEMNT_ID & $tADR_HOME & $tADR_OTHER & $tADR_WORK & $tAGENT & $tATTACHMENTS & $tBDAY & $tEMAIL_OTHER_INTERNET & $tEMAIL_PREF_INTERNET & _
				...
				$tTEL_CELL_VOICE & $tTEL_HOME_VOICE & $tTEL_PAGER_VOICE & $tTEL_WORK_FAX & $tTEL_WORK_VOICE & $tTITLE & $tURL_WORK & $tELEMENT_END
	Next
