...
#AutoIt3Wrapper_Outfile=Builds\GoogleContactsToMailEnable_v1.1.exe
#AutoIt3Wrapper_Outfile_x64=Builds\GoogleContactsToMailEnable_64-bit(x64)_v1.1.exe
#AutoIt3Wrapper_Res_Comment=By Phoenix125.com
#AutoIt3Wrapper_Res_Description=Google Contacts vCard to MailEnable Importer
#AutoIt3Wrapper_Res_Fileversion=1.1
#AutoIt3Wrapper_Res_ProductName=Google Contacts To MailEnable Importer
...
#EndRegion
Global $aUtilName = "Google Contacts to MailEnable"
...
EndIf
FileInstall("K:\AutoIT\_MyProgs\GoogleContactsToMailEnable\Resources\phoenixlogo.jpg", $aFolderTemp, 0)
FileInstall("K:\AutoIT\_MyProgs\GoogleContactsToMailEnable\Resources\MailEnableLogo1.jpg", $aFolderTemp, 0)
FileInstall("K:\AutoIT\_MyProgs\GoogleContactsToMailEnable\Resources\GoogleLogo.jpg", $aFolderTemp, 0)
FileInstall("K:\AutoIT\_MyProgs\GoogleContactsToMailEnable\Resources\GoogleExport1.jpg", $aFolderTemp, 0)
...
Else
If $tPost = "" Then
If $aReformatPhoneNumbersYN = "yes" Then $tNextTemp = _FormatPhoneNumber($tNextTemp)
$gCustom[$tNo] &= "Work:" & $tNextTemp & @CRLF
Else
If $aReformatPhoneNumbersYN = "yes" Then $tPost = _FormatPhoneNumber($tPost)
$gCustom[$tNo] &= "Work:" & $tPost & @CRLF
EndIf
EndIf
...
Else
$gCustom[$tNo] &= "Home:" & @CRLF
If UBound($tSplit) > 1 Then
If $tSplit[1] <> "" Then $gCustom[$tNo] &= $tSplit[1] & @CRLF
EndIf
If UBound($tSplit) > 2 Then
If $tSplit[2] <> "" Then $gCustom[$tNo] &= $tSplit[2] & @CRLF
EndIf
If UBound($tSplit) > 3 Then
If $tSplit[3] <> "" Then $gCustom[$tNo] &= $tSplit[3] & @CRLF
EndIf
If UBound($tSplit) > 4 Then
If $tSplit[4] <> "" Then $gCustom[$tNo] &= $tSplit[4] & ", "
EndIf
If UBound($tSplit) > 5 Then
If $tSplit[5] <> "" Then $gCustom[$tNo] &= $tSplit[5] & " "
EndIf
If UBound($tSplit) > 6 Then
If $tSplit[6] <> "" Then $gCustom[$tNo] &= $tSplit[6] & " "
EndIf
If UBound($tSplit) > 7 Then
If $tSplit[7] <> "" Then $gCustom[$tNo] &= $tSplit[7] & @CRLF
EndIf
EndIf
ElseIf $tPre = "ADR;TYPE=WORK" Then
Local $tSplit = StringSplit($tPost, ";")
If $gAddressWorkCity[$tNo] = "" Then
...
$tCnt += 1
If $tCnt = 10 Then
ControlSetText($aSplash, "", "Static1", "Converting to MailEnable files." & @CRLF & "Processing contact " & $tNo & " of " & $aNumberOfContacts & @CRLF & @CRLF & "Timer:" & Round(_Timer_Diff($aTimerStart) / 1000) & "s")
$tCnt = 0
EndIf
Local $tAdd2Home = ""
Local $tAddPOHome = ""
...
If $gHomePage[$tNo] <> "" Then
If $mURL_WORK[$tNo] = "" Then
$mURL_WORK[$tNo] = $gHomePage[$tNo]
Else
$mNOTE[$tNo] &= "Home Page:" & $gHomePage[$tNo] & @CRLF
EndIf
EndIf
If $gURLBlog[$tNo] <> "" Then
If $mURL_WORK[$tNo] = "" Then
$mURL_WORK[$tNo] = $gURLBlog[$tNo]
Else
$mNOTE[$tNo] &= "Blog:" & $gURLBlog[$tNo] & @CRLF
EndIf
EndIf
If $gURLProfile[$tNo] <> "" Then
If $mURL_WORK[$tNo] = "" Then
$mURL_WORK[$tNo] = $gURLProfile[$tNo]
Else
$mNOTE[$tNo] &= "Profile:" & $gURLProfile[$tNo] & @CRLF
EndIf
EndIf
$mURL_WORK[$tNo] = StringReplace($gURLWork[$tNo], "\:", ":")
...
$tReturn = StringReplace($tTxt, "\n", @CRLF)
$tReturn = StringReplace($tTxt, "http\:", "http:")
Return $tReturn
EndFunc
...
For $tNo = 0 To ($aNumberOfContacts - 1)
Local $tArray[0]
ControlSetText($aSplash, "", "Static1", "Writing MailEnable files." & @CRLF & "Processing contact " & $tNo & " of " & $aNumberOfContacts & @CRLF & @CRLF & "Timer:" & Round(_Timer_Diff($aTimerStart) / 1000) & "s")
...
ElseIf $mTEL_WORK_FAX[$tNo] <> "" Then
$mTEL_CELL_VOICE[$tNo] = $mTEL_WORK_FAX[$tNo]
EndIf
EndIf
EndIf
...
If $mTEL_WORK_FAX[$tNo] <> "" Then $tTEL_WORK_FAX = '<TEL_WORK_FAX>' & $mTEL_WORK_FAX[$tNo] & '</TEL_WORK_FAX>'
...
If $mURL_WORK[$tNo] <> "" Then $tURL_WORK = '<URL_WORK>' & $mURL_WORK[$tNo] & '</URL_WORK>'
$tELEMENT_END = '</ELEMENT>'
$mIndex[$tNo] = $tELEMNT_ID & $tADR_HOME & $tADR_OTHER & $tADR_WORK & $tAGENT & $tATTACHMENTS & $tBDAY & $tEMAIL_OTHER_INTERNET & $tEMAIL_PREF_INTERNET & $tEMAIL_WORK_INTERNET & $tFN & $tIMPORTANCE & $tMODIFIEDDATE & $tIM_TYPE_MSMESSENGER & $tN & $tNICKNAME & $tORG & $tREAD & $tSIZE & $tSTATE & $tTEL_CELL_VOICE & $tTEL_HOME_VOICE & $tTEL_PAGER_VOICE & $tTEL_WORK_FAX & $tTEL_WORK_VOICE & $tTITLE & $tURL_WORK & $tELEMENT_END
Next
...
IniWrite($aIniFile, $aINIHeader, "CheckForUpdatesYN", $aCheckForUpdatesYN)
_Splash("Utility update check disabled." & @CRLF & "To enable update check, change" & @CRLF & "[CheckForUpdatesYN=yes] in the .ini.", 5000, 500)
ElseIf $tMB = 2 Then
EndIf
EndIf
EndIf
...
Global $W_MainWindow = GUICreate("Google Contacts to MailEnable", 902, 578, -1, -1, BitOR($GUI_SS_DEFAULT_GUI, $WS_SIZEBOX, $WS_THICKFRAME))
...
Global $L_Title = GUICtrlCreateLabel("Google Contacts to MailEnable", 228, 31, 484, 41)
...
Global $G_OutputGroup = GUICtrlCreateGroup("Output: MailEnable Mailbox Contact Folder", 22, 363, 857, 115)
GUICtrlSetFont(-1, 9, 400, 0, "MS Sans Serif")
GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
...
Global $L_SelectContactsFolder = GUICtrlCreateLabel("Select MailEnable Mailbox Contact Folder:", 33, 392, 352, 23)
GUICtrlSetFont(-1, 12, 800, 2, "Tahoma")
GUICtrlSetColor(-1, 0xFFFF00)
GUICtrlSetResizing(-1, $GUI_DOCKLEFT + $GUI_DOCKTOP + $GUI_DOCKWIDTH + $GUI_DOCKHEIGHT)
...
Local $tExportFolder = FileSelectFolder("Please select MailEnable Contact (Output) Folder.", $aExportFolder)
If @error Then
GUICtrlSetData($I_OutputContactsFolder, $aExportFolder)
Else
